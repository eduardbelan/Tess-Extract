...
# extracted_txt = pytesseract.image_to_string(PIL.Image.open("umweltschutz-arbeitsschutz.png"), config=myconfig)
# print(extracted_txt)
...
# Loop over each line of text (each detected character) in the result returned by pytesseract.image_to_boxes() method
# for box in boxes.splitlines():
    # Split the line of text (detected character) into a list of substrings using the split() method with space (" ")
    # box = box.split(" ")
...
for i in range(amount_boxes):
    ...
    if float(data["conf"][i]) > 0:
        ...
        # Print the text of the detected word above the rectangle in red
